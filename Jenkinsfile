      ...
      post {
                success {
                    ...
                    script {
                      ...
                      if (json.iid != null){
                      def iid = json.iid
                      final String resp = sh(script: 'curl -X POST "https://gitlab.com/api/v4/projects/19079864/$idd/approve" --header "Authorization: Bearer Gpzv8Cn-AFFJitgYuSAX"', returnStdout: true).trim()
                      echo resp
                      }
                    }
                }
