    ...
    stage('Create MR') {
      ...
      post {
                success {
                    ...
                    script {
                      final String response = sh(script: 'curl -X POST "https://gitlab.com/api/v4/projects/19079864/merge_requests?source_branch=merge-request&target_branch=master&title=test&target_project_id=19079864" --header "Authorization: Bearer TkB3p7BB5PLtuEgc86wH"', returnStdout: true).trim()
                      echo response
                      def json = new groovy.json.JsonSlurperClassic().parseText(response)
                      if (json.iid != null){
                      def iid = json.iid.toString()
                      echo iid
                      final String resp = sh(script: "curl -X PUT https://gitlab.com/api/v4/projects/19079864/merge_requests/$iid/merge --header Authorization: Bearer TkB3p7BB5PLtuEgc86wH", returnStdout: true).trim()
                      echo resp
                      }
                    }
                }
